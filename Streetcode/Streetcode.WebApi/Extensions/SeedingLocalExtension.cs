...
namespace Streetcode.WebApi.Extensions
{
    public static class SeedingLocalExtension
    {
        public static async Task SeedDataAsync(this WebApplication app)
        {
            using (var scope = app.Services.CreateScope())
            {
                ...
                if (!await dbContext.Images.AnyAsync())
                {
                    ...
                    await SeedTermsAndRelatedTermsAsync(dbContext);

                    await SeedTeamMemberDataAsync(dbContext);

                    await SeedRolesAsync(roleManager);

                    await SeedUsersAsync(dbContext);

                    await SeedNewAsync(dbContext);
                    ...
                    if (!await dbContext.Audios.AnyAsync())
                    {
                        ...
                        await SeedStreetcodeDataAsync(dbContext);
                    }
        ...
        private static async Task SeedResponsesAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedTermsAndRelatedTermsAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedTeamMemberDataAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedUsersAsync(StreetcodeDbContext dbContext)
        {
            if (!await dbContext.Users.AnyAsync(u => u.Email == "main_admin@gmail.com"))
            {
            ...
            if (!await dbContext.Users.AnyAsync(u => u.Email == "admin@gmail.com"))
            {
            ...
            if (!await dbContext.Users.AnyAsync(u => u.Email == "moderator@gmail.com"))
            {
        ...
        private static async Task SeedNewAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedSubtitlesAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedStreetcodeCoordinatesAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedVideosAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedPartnersDataAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedArtsDataAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedTextsAsync(StreetcodeDbContext dbContext)
        {
    ...
    Як попихач носив воду, опалював школу, обслуговував дяка, читав псалтир над померлими і вчився. Не стерпівши знущань Богорського й відчуваючи великий потяг до живопису, Тарас утік від дяка й почав шукати в навколишніх селах учителя-маляра. Кілька днів наймитував і «вчився» малярства в диякона Єфрема. Також мав учителів-малярів із села Стеблева, Канівського повіту та із села Тарасівки Звенигородського повіту. 1827 року він пас громадську отару в Кирилівці й там зустрічався з Оксаною Коваленко. Згодом подругу свого дитинства поет не раз згадає у своїх творах і присвятить їй поему «Мар'яна-черниця».",
        ...
        private static async Task SeedTimelineDataAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedTransactionLinksAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedFactsDataAsync(StreetcodeDbContext dbContext)
        {
            if (!await dbContext.Facts.AnyAsync())
            {
                dbContext.Facts.AddRange(
                    new Fact
                    {
                        ...
                        ImageDescription = "Портрет Василя Жуковського",
                        ImageId = 6,
                        StreetcodeId = 1,
                        Order = 1,
                    },
                    new Fact
                    {
                        ...
                        ImageDescription = "Обкладинка першого видання Кобзаря",
                        ImageId = 5,
                        StreetcodeId = 1,
                        Order = 2,
                    },
                    new Fact
                    {
                        ...
                        ImageDescription = "Фото премії Романа Ратушного",
                        ImageId = 16,
                        StreetcodeId = 2,
                        Order = 1,
                    },
                    new Fact
                    {
                        ...
                        ImageDescription = "Стипендія для активістів",
                        ImageId = 17,
                        StreetcodeId = 2,
                        Order = 2,
                    },
                    new Fact
                    {
                        Title = "Активіст, який не зупинявся ні перед чим",
                        FactContent = "Роман Ратушний ще підлітком усвідомив, що громадянська позиція — це щоденна дія. Він став одним із ключових захисників Протасового Яру в Києві, перетворивши локальний конфлікт на загальнонаціональний символ боротьби за право міста на зелені зони та гідне майбутнє.",
                        ImageDescription = "Роман Ратушний під час громадської акції на захист Протасового Яру",
                        ImageId = 18,
                        StreetcodeId = 2,
                        Order = 3,
                    },
                    new Fact
                    {
                        ...
                        ImageDescription = "Карта мафіозних зв’язків",
                        ImageId = 19,
                        StreetcodeId = 2,
                        Order = 4,
                    },
                    ...
                    new Fact
                    {
                        ...
                        ImageDescription = "Каска Романа Ратушного з Майдану",
                        ImageId = 17,
                        StreetcodeId = 2,
                        Order = 6,
                    },
                    new Fact
                    {
                        ...
                        ImageDescription = "Роман Ратушний з батьками",
                        ImageId = 18,
                        StreetcodeId = 2,
                        Order = 7,
                    },
        ...
        private static async Task SeedSourceLinksAndStreetcodeCategoriesAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedRelatedFiguresAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedStreetCodeImagesAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedTagsDataAsync(StreetcodeDbContext dbContext)
        {
        ...
        private static async Task SeedStreetcodeDataAsync(StreetcodeDbContext dbContext)
        {
            if (!await dbContext.Streetcodes.AnyAsync())
            {
                ...
                await SeedPartnersDataAsync(dbContext);

                await SeedArtsDataAsync(dbContext);

                await SeedTextsAsync(dbContext);

                await SeedTimelineDataAsync(dbContext);

                await SeedTransactionLinksAsync(dbContext);

                await SeedFactsDataAsync(dbContext);

                await SeedSourceLinksAndStreetcodeCategoriesAsync(dbContext);

                await SeedRelatedFiguresAsync(dbContext);

                await SeedStreetCodeImagesAsync(dbContext);

                await SeedTagsDataAsync(dbContext);
            }
        }
    }
}
