namespace Streetcode.XUnitTest.MediatR.Timeline.TimelineItem.Create
{
    ...
    {
        private readonly CreateTimelineItemDtoValidator validator;

        public CreateTimelineItemDtoValidatorTests()
        {
            this.validator = new CreateTimelineItemDtoValidator();
        }

        private static CreateTimelineItemDto CreateValidDto() =>
            new CreateTimelineItemDto
            {
                Title = "Valid Title",
        ...
        {
            // Arrange
            var dto = new CreateTimelineItemDto
            {
                Title = null!,
            ...
            // Assert
            Assert.True(result.Errors.Count >= 6); // At least 6 errors expected
            result.ShouldHaveValidationErrorFor(x => x.Title);
            result.ShouldHaveValidationErrorFor(x => x.Description);
            result.ShouldHaveValidationErrorFor(x => x.Date);
            result.ShouldHaveValidationErrorFor(x => x.DateViewPattern);
