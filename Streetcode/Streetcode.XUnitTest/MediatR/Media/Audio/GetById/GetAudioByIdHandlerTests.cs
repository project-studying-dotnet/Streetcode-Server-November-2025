namespace Streetcode.XUnitTest.MediatR.Media.Audio.GetById
{
    ...
    public class GetAudioByIdHandlerTests
    {
        ...
        private static (Audio Audio, AudioDtoo AudioDTO, int TargetAudioId) CreateValidAudioEntityAndDTO()
        {
        ...
        private static (Audio? Audio, AudioDtoo? AudioDTO, int TargetAudioId) CreateNullAudioEntity()
        {
            const int targetAudioId = 1;

            Audio? entity = null;
            AudioDtoo? mappedDTO = null;

            return (entity, mappedDTO, targetAudioId);
        }

        private void SetupMocks(Audio? entity, AudioDtoo? audioDTO)
        {
            this.mockRepo
            ...
            {
                this.mockMapper
                    .Setup(m => m.Map<AudioDtoo>(It.IsAny<Audio>()))
                    .Returns(audioDTO);
