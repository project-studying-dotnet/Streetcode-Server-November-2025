namespace Streetcode.XUnitTest.MediatR.Media.Audio.GetAll
{
    ...
    public class GetAllAudiosHandlerTests
    {
        ...
        private static (IEnumerable<Audio> AudioEntities, IEnumerable<AudioDtoo> AudioDtos) CreateValidAudioEntitiesAndDTOs()
        {
            ...
            var audioDtos = new List<AudioDtoo>
            {
                new AudioDtoo { Id = 1, Base64 = "base64string1" },
                new AudioDtoo { Id = 2, Base64 = "base64string2" },
            };
            return (audioEntities, audioDtos);
        }

        private static (IEnumerable<Audio>? AudioEntities, IEnumerable<AudioDtoo>? AudioDtos) CreateNullAudioEntitiesAndDTOs()
        {
            List<Audio>? audioEntities = null;
            List<AudioDtoo>? audioDtos = null;

            return (audioEntities, audioDtos);
        }

        private void SetupMock(IEnumerable<Audio> audioEntities, IEnumerable<AudioDtoo> audioDtos)
        {
            ...
            this.mockMapper
                .Setup(m => m.Map<IEnumerable<AudioDtoo>>(It.IsAny<IEnumerable<Audio>>()))
                .Returns(audioDtos);
        ...
        private void VerifyMocksCalledOnce()
        {
            ...
            this.mockMapper.Verify(
                m => m.Map<IEnumerable<AudioDtoo>>(It.IsAny<IEnumerable<Audio>>()),
                Times.Once);
