...
{
    public class SortedByDateTimeHandler : IRequestHandler<SortedByDateTimeQuery, Result<List<NewsDtoo>>>
    {
        ...
        public async Task<Result<List<NewsDtoo>>> Handle(SortedByDateTimeQuery request, CancellationToken cancellationToken)
        {
            ...
            var newsDTOs = _mapper.Map<IEnumerable<NewsDtoo>>(news).OrderByDescending(x => x.CreationDate).ToList();
